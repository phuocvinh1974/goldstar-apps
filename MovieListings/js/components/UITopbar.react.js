...
var UITopbar = React.createClass({
	render: function () {
		...
		return (
			<div style={assign(baseStyle,{WebkitJustifyContent:'space-between'})}>
				<div style={baseStyle}>
					<i onClick={this._menuDrawerToggle} className="material-icons" style={{marginLeft:10,cursor:'pointer'}}>menu</i>
				...
				<div style={baseStyle}>
					...
					<i onClick={this._menuMore} className="material-icons" style={{marginRight:10,cursor:'pointer'}}>more_vert</i>
				</div>
			</div>
		);
	},
	...
	// ACTIONS
	_menuDrawerToggle: function () {
		UIActions.menuDrawerToggle();
	},
	_menuMore: function (e) {
		UIActions.menuMoreToggle(e.target.getBoundingClientRect());
	}
});
